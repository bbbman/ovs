...
//    $(".upload").upload({ 
//        uploadData: { caption: "12233" }, 
//        successFn: "success",           //可以是字符串 
//    }); 
//});
function getFlag(){
	var imageIds = document.getElementsByName("imageId");
	for(var i=0;i<imageIds.length;i++){
		if(imageIds[i].getAttribute("flag") == 1){
			imageIds[i].setAttribute("flag",0);
			return imageIds[i];
		}
	}
}
function uploadMyImage(which){
	which.setAttribute("flag",1);
	var ttest = $(".upload").upload({ 
        uploadData: { caption: "12233" }, 
        successFn: "success",           //可以是字符串 
    });
}
...
function success(response, statusText, xhr, $this) { 
    //比如插入编辑器 
    //alert(response.imagePath + $this.attr("value")) 
	alert("上传成功");
} 
    ...
    $.extend($.fn, {  
        upload: function (settings) {  
            var options = $.extend({  
                ...
                uploadUrl: "http://localhost:8080/ovs/common/file-up-load!doee.action",      //上传URL地址  
                width: 150,                                              //图片显示的宽度  
                height: "",                                            //图片显示的高度  
...
//  
//                            if (showMessage) { alert("2"+response.message) }  
//  
...
//                    if ($fileInput.val() !== "") { 
//                    	 $fileInput.val("");
//                         $this.parent().find(options.imgSelector).find("img").remove();
...
//                    }  
                    ...
                    data.error = function (response, statusText, xhr, $form) {  
                        ...
                        if (errorFn) {  
                            errorFn(response.message, statusText, xhr, $this);  
                        }  
                    };  
                    //上传成功  
                    data.success = function (response, statusText, xhr, $form) {  
                        ...
                        if (response.status == '1') { 
                        	var divP = getFlag();
                        	divP.setAttribute("value",response.imageId);
                        	
                            methods.showImage(response.imagePath, $this.parent());  
                            $this.parent().find("input:hidden").val(response.imagePath);  
                            var successFn;  
                        ...
                        fileBox.html("<input type=\"file\" name=\"myImage\" style=\"margin-left:23%;\" />");  
                        $form.remove();
                        return response;
                    };  
